...
model_name=TimeMoE_original_fix_200M
seq_len=512
pred_len=288

python -u run.py \
  --task_name zero_shot_forecast \
  --des 'DemandLoadForecastCommonA100' \
  --is_training 0 \
  --is_testing 0 \
  --is_forecasting 1 \
  ...
  --time date \
  --model_id df_power_forecast_common_a100_$seq_len'_'$pred_len \
  --model $model_name \
  --data custom \
  --features S \
  --target h_total_use \
  ...
  --itr 1 \
  --pretrain_checkpoints ./pretrain_models/TimeMoE-200M \
