...
def build_series_windows(
    ...
    if len(values) < total_length:
        raise ValueError(
            f"dataset length {len(values)} is shorter than required window size {total_length}"
        )
    ...
    targets = np.stack(
        [values[start + context_length:start + total_length] for start in starts]
    ).astype(np.float32)
